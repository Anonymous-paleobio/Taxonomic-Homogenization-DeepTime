...
geoIntervals <- read.delim(paste("https://paleobiodb.org/data1.2/intervals/list.tsv?scale=1&max_ma=",maxAge$b_age[1], sep=""))
geoIntervals <- subset(geoIntervals, type %in% "age")
names(geoIntervals)[names(geoIntervals) == 'b_age'] <- 'max_ma'
names(geoIntervals)[names(geoIntervals) == 't_age'] <- 'min_ma'
...
for(i in 1:nrow(geoIntervals)) {
  ...
  pbdbUri <- URLencode(paste("https://paleobiodb.org/data1.2/occs/list.tsv?all_records&base_name=Animalia&interval=",geoIntervals$interval_name[i],"&show=paleoloc,classext,env&idreso=lump_gensub&limit=all", sep=""))
  #also include enviornent
...
save(pbdb, genera, geoIntervals, file="pbdbAnimalsNov2025.Rdata")

#read in data files from Paleosize Database#
data.pbdb <- pbdb #PBDB data

write.csv(data.pbdb, file="pbdb.data.Nov2025.csv")
